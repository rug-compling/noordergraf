...
SELECT DISTINCT ?item ?name {
  ?item :quote / :reference / :book bible:` + book + ` .
  ?item :subject / :name / :fullName ?name .
} ORDER BY ?name ?item
`
	...
	title := []string{"Graven met verwijzingen naar bijbelboek", "Tombs with references to bible book"}[lang]
...
%s: %d
<table>
`, langtag, title, book, title, book, found, len(sparql.Results))
